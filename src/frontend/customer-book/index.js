...
const Customer = ({ users, createRecord, updateRecord, deleteRecord, user }) => {

    const [customer, setCustomer] = useState({ _id: null, name: '' })
    ...
    const handleSubmit = async e => {
        ...
        if (customer.name.length) {
            setPending(true)
            const resp = await (!customer._id ? createRecord({ ...customer, name: `${customer.name.replaceAll('-', " ").trim()}-${user?._id}` }) : updateRecord({ ...customer, name: `${customer.name.replaceAll('-', " ").trim()}-${user?._id}` }))
            if (resp) {
                setUsers(resp)
                return
            }
